/*
  # Add Exa.ai Integration for Real-time Alerts

  1. New Tables
    - `exa_api_cache` - Stores cached API responses to reduce API calls
    - `exa_search_history` - Tracks search history for analytics
  2. Security
...
CREATE POLICY "Service role can insert/update exa_api_cache" ON exa_api_cache
...
FOR INSERT USING (auth.role() = 'service_role' OR auth.uid() = user_id);
