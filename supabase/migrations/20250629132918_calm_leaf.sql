...
-- Service role policies with separate INSERT/UPDATE/DELETE
CREATE POLICY "Service role can insert to exa_api_cache" ON exa_api_cache
FOR INSERT WITH CHECK (role() = 'service_role');
...
FOR DELETE USING (role() = 'service_role');

CREATE POLICY "Service role can insert search history" ON exa_search_history
FOR INSERT WITH CHECK (role() = 'service_role' OR auth.uid() = user_id);
